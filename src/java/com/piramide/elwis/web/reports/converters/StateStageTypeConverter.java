 ...
 */
public class StateStageTypeConverter implements Converter {
    ...
    public FieldValue dbToView(Field field, Map map, Pair[] pairs, ResourceBundleWrapper resourceBundleWrapper, String titusPath) {
        ...
        if (fieldPairValue != null && !"".equals(fieldPairValue)) {
            ...
            if (("0").equals(opeValue)) {
                fvObj = new FieldValue(resourceBundleWrapper.getMessage("State.stage.open"), fieldPairValue);
            } else if (("1").equals(opeValue)) {
                fvObj = new FieldValue(resourceBundleWrapper.getMessage("State.stage.progress"), fieldPairValue);
            } else if (("2").equals(opeValue)) {
                fvObj = new FieldValue(resourceBundleWrapper.getMessage("State.stage.close"), fieldPairValue);
            }
