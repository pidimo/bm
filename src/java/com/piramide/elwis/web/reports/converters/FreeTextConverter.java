...
public class FreeTextConverter implements Converter {
    ...
    public FieldValue dbToView(Field field, Map map, Pair[] pairs, ResourceBundleWrapper resourceBundleWrapper, String titusPath) {
        ...
        if (fieldPairValue != null && !"".equals(fieldPairValue)) {
            if (fieldPairValue instanceof Blob) {
                try {
                    aByte = (Blob) fieldPairValue;
                    fvObj = new FieldValue(new String(aByte.getBytes(1, (int) aByte.length())), fieldPairValue);
                } catch (SQLException e) {
                    e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
                }
            } else {
                fvObj = new FieldValue(fieldPairValue, fieldPairValue);
            }
        } else {
