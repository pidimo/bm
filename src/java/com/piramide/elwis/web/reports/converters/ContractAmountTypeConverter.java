 ...
 *
 * @author Miky
 * @version $Id: ContractAmountTypeConverter.java 11-feb-2009 16:09:24 $
 */
public class ContractAmountTypeConverter implements Converter {
    ...
    public FieldValue dbToView(Field field, Map map, Pair[] pairs, ResourceBundleWrapper resourceBundleWrapper, String titusPath) {
        ...
        if (fieldPairValue != null && !"".equals(fieldPairValue)) {
            ...
            if (SalesConstants.AmounType.AMOUNT.getConstantAsString().equals(rowValue)) {
                fvObj = new FieldValue(resourceBundleWrapper.getMessage("ProductContract.amountType.amount"), fieldPairValue);
            } else if (SalesConstants.AmounType.PERCENTAGE.getConstantAsString().equals(rowValue)) {
                fvObj = new FieldValue(resourceBundleWrapper.getMessage("ProductContract.amountType.percentage"), fieldPairValue);
            }
        } else {
