...
public class DiscountTypeConverter implements Converter {
    ...
    public FieldValue dbToView(Field field, Map map, Pair[] pairs, ResourceBundleWrapper resourceBundleWrapper, String titusPath) {
        ...
        if (fieldPairValue != null && !"".equals(fieldPairValue)) {
            ...
            if (("1").equals(opeValue)) {
                fvObj = new FieldValue(resourceBundleWrapper.getMessage("SalesProcessAction.percent_discount"), fieldPairValue);
            } else if (("2").equals(opeValue)) {
                fvObj = new FieldValue(resourceBundleWrapper.getMessage("SalesProcessAction.amount_discount"), fieldPairValue);
            }
        } else {
