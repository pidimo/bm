...
public class PredeterminedConverter implements Converter {
    ...
    public FieldValue dbToView(Field field, Map map, Pair[] pairs, ResourceBundleWrapper resourceBundleWrapper, String titusPath) {
        ...
        if (fieldPairValue != null && !"".equals(fieldPairValue)) {
            ...
            if (new Integer(0).equals(opeValue)) {
                fvObj = new FieldValue(resourceBundleWrapper.getMessage("Common.alternative"), fieldPairValue);
            } else if (new Integer(1).equals(opeValue)) {
                fvObj = new FieldValue(resourceBundleWrapper.getMessage("Common.predetermined"), fieldPairValue);
            }
        } else {
