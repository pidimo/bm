 ...
 * @version 0.2
 */
public class InvoiceCustomerDataExportReportAction extends CustomCSVExportReportAction {
    private Log log = LogFactory.getLog(this.getClass());

    @Override
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        log.debug("Executing InvoiceCustomerDataExportReportAction.... " + request.getParameterMap());
    ...
    protected String getFileName(ReportData reportData, HttpServletRequest request) {
        String fileName = JSPHelper.getMessage(request, "Finance.report.invoiceCustomerDataExport.fileName", composeRangeDate(request));
    ...
    private ActionErrors exportDataValidation(ReportGeneratorForm searchForm, HttpServletRequest request) {
        ...
        List<String> customerWithoutNumber = InvoiceCSVExportReportUtil.getCustomersWithoutCustomerNumber(
                getSearchParameterAsInt(searchForm, "startInvoiceDate"),
                getSearchParameterAsInt(searchForm, "endInvoiceDate"),
                request);

        if (!customerWithoutNumber.isEmpty()) {
            errors.add("withoutNumber", new ActionError("InvoiceExport.error.msg"));
            request.setAttribute("customerWithoutNumberList", customerWithoutNumber);
        }
        return errors;
    }
}
