 ...
 * @version 0.2
 */
public class InvoicePartnerPaymentExportReportAction extends CustomCSVExportReportAction {
    private Log log = LogFactory.getLog(this.getClass());

    @Override
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        log.debug("Executing InvoicePartnerPaymentExportReportAction.... " + request.getParameterMap());
    ...
    protected String getFileName(ReportData reportData, HttpServletRequest request) {
        String fileName = JSPHelper.getMessage(request, "Finance.report.invoicePartnerPaymentExport.fileName", composeRangeDate(request));
    ...
    private ActionErrors exportDataValidation(ReportGeneratorForm searchForm, HttpServletRequest request) {
        ...
        Map errorDataResultMap = InvoiceCSVExportReportUtil.validateInvoicePaymentExportedData(
                ...
                request,
                InvoiceCSVExportReportUtil.EXPORTDATATYPE_PARTNER);
