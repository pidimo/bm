 ...
 */
public class TaskForwardAction extends com.piramide.elwis.web.salesmanager.action.SalesProcessManagerAction {
    ...
    public ActionForward execute(ActionMapping action, ActionForm form, HttpServletRequest request,
        ...
        if (!cmd.getResultDTO().isFailure()) {
            ...
            setDefaultNavigationOption((DefaultForm) form);
    ...
    private void setDefaultNavigationOption(DefaultForm defaultForm) {
        if (null != defaultForm.getDto("processId")
                && !"".equals(defaultForm.getDto("processId").toString().trim())) {
            defaultForm.setDto("navigationOption", "2");
        }
    }
}