...
public abstract class TableBaseBootstrapTag extends BodyTagSupport {

    protected static Log log = LogFactory.getLog("TableBaseTag");

    public final static String recurrenceIMGIcon = "glyphicon glyphicon-retweet";
    public final static String reminderIMGIcon = "glyphicon glyphicon-bell";
    public final static String addIMGIcon = "glyphicon glyphicon-plus";
    public final static String delIMGIcon = "glyphicon glyphicon-trash";
    public final static String ownerIMGIcon = "glyphicon glyphicon-user";
    public final static String participantIMGIcon = "fa fa-users";
    ...
    private final static String styleChangeColor = "colorAHref";
    private final static String styleButtonDefault = "btn btn-default";

    public final static String glypgiconNext="glyphicon glyphicon-arrow-right";
    public final static String glypgiconPrevius="glyphicon glyphicon-arrow-left";
    ...
    protected static final String TABLE_CSS = "table table-striped table-hover table-bordered calendar_table";
    ...
    public String renderGlyphiconPrevius() {
        StringBuffer html = new StringBuffer();
        html.append("<i class=\"").append(glypgiconPrevius).append("\"></i>");

        return html.toString();
    }
    public String renderGlyphiconNext() {
        StringBuffer html = new StringBuffer();
        html.append("<i class=\"").append(glypgiconNext).append("\"></i>");
    ...
    protected void renderPreviousButton(StringBuffer html, String url, String text, String tip, String classCSS, String otherAttrib) {

        HttpServletResponse response = (HttpServletResponse) pageContext.getResponse();
        html.append("<span class=\"input-group-btn\"><a href=\"").append(response.encodeURL(url)).append("\" ");
        ...
        html.append(">").append(text).append("</a></span>");
    }
    protected void renderNextButton(StringBuffer html, String url, String text, String tip, String classCSS, String otherAttrib) {
    ...
    protected void renderIcons(StringBuffer html, String calendarDate, AppointmentCompountView appointment) {
        renderOpenSPAN(html, appointment.isOwner() ? ownerIMGIcon : participantIMGIcon,appointment.isOwner() ? createProperty(PROPERTY_TITLE,RES_OWNER) : createProperty(PROPERTY_TITLE,RES_PARTICIPANT) );
        ...
        if (appointment.isReminder()) {
            ...
            if (remType >= 0 && remType < 3) {
                ...
                renderOpenSPAN(html, reminderIMGIcon, createProperty(PROPERTY_TITLE, tipReminder));
                renderCloseSPAN(html);
                html.append(SPACE);
            }
        }
        ...
        if (appointment.isRecurrent()) {
            renderOpenSPAN(html, recurrenceIMGIcon, createProperty(PROPERTY_TITLE,RES_RECURRENCE) );
            renderCloseSPAN(html);
            html.append(SPACE);
        }
    ...
    protected void renderTimeLabelAndIcons(StringBuffer html, String calendarDate, AppointmentCompountView appointment, boolean onlyAnonymous, boolean showToolTip) {
        HttpServletRequest request = (HttpServletRequest) pageContext.getRequest();
        ...
        if (onlyAnonymous) {
            ...
            String modurl = composeModURL(request, calendarDate, appointment);
            String otherAttrib = null;

            if (showToolTip) {
                otherAttrib = renderToolTip(TOOLTIP_WITDH, appointment);
            }
            renderHREF(html, modurl, timeLabel.toString(), null, styleChangeColor, otherAttrib);
        }
        ...
        //recurrent icon
        if (appointment.isRecurrent()) {
            renderOpenSPAN(html, recurrenceIMGIcon + " " + getTimeLabelIconClass(), createProperty(PROPERTY_TITLE, RES_RECURRENCE));
            renderCloseSPAN(html);
        }
        ...
        if (appointment.isReminder()) {
            ...
            if (remType >= 0 && remType < 3) {
                tipReminder = RES_REMINDER.replaceAll("<type>", RES_REMINDER_TYPE[remType]);
                tipReminder = tipReminder.replaceAll("<time>", appointment.getTimeBefore());
                renderOpenSPAN(html, reminderIMGIcon + " " + getTimeLabelIconClass(), createProperty(PROPERTY_TITLE, tipReminder));
                renderCloseSPAN(html);
            }
        }

        //owner or participant icon
        renderOpenSPAN(html, (appointment.isOwner() ? ownerIMGIcon : participantIMGIcon) + " " + getTimeLabelIconClass(), appointment.isOwner() ? createProperty(PROPERTY_TITLE, RES_OWNER) : createProperty(PROPERTY_TITLE, RES_PARTICIPANT));
        renderCloseSPAN(html);
    }
    ...
    protected String renderToolTip(String width, AppointmentView appointment) {
        ...
        StringBuffer tooltip = new StringBuffer("onmouseover=\"Tip(myToolTip('").append(title);
        ...
        tooltip.append("', '")
                ...
                .append(location)
                .append("'),")
                .append("BGCOLOR, '#FFFFBB')\"")
                .append("onmouseout=\"UnTip()\"");
    ...
    protected void renderAddURL(HttpServletRequest req, StringBuffer html, String text, int hour, String min, String date, String calendarDate) {
        if (hasAddAppointmentPermission()) {
            ...
            if (text == null) {
                ...
                renderOpenSPAN(res, addIMGIcon, createProperty(PROPERTY_TITLE, RES_APPOINTMENT_ADD));
                renderCloseSPAN(res);
                text = res.toString();
            }
    ...
    protected String composeModURL(HttpServletRequest req, String calendarDate, AppointmentCompountView appointment) {
    ...
    protected void renderModURL(HttpServletRequest req, StringBuffer html, StringBuffer time, String calendarDate, AppointmentCompountView appointment, boolean chop, boolean showToolTip) {
        ...
        String modurl = composeModURL(req, calendarDate, appointment);
        ...
        renderHREF(html, modurl, time.toString(), null, styleChangeColor, otherAttrib);
    }
    ...
    protected void renderDelURL(HttpServletRequest req, StringBuffer html, String calendarDate, AppointmentCompountView appointment) {
        ...
        if (hasDelAppointmentPermission(appointment.isPublic()) && appointment.isOwner()) {
            ...
            renderOpenSPAN(img, delIMGIcon + " " + getTimeLabelIconClass(), createProperty(PROPERTY_TITLE, RES_APPOINTMENT_DEL));
            renderCloseSPAN(img);
    ...
    protected void renderOpenSPAN(StringBuffer html, String classCSS) {
        html.append("<span class=\"").append(classCSS).append("\"").append(" style=\"background-color: transparent\"").append(">");
    }
    ...
    private String createProperty(String nameAttribute, String value){
        StringBuffer aux = new StringBuffer();
        aux.append(nameAttribute).append("=\"").append(value).append("\"");
        return aux.toString();
    }
...
StringBuffer().append(request.getContextPath()).append(url).append(getType()).append("&calendar=").append(value).toString();
       ...
       */
        ...
        renderOpenDIV(html, "form-inline", null);
        html.append("<div class=\"row col-xs-12 col-sm-4 col-md-3 col-lg-3 wrapperButton\">");
        renderOpenDIV(html, "input-group", null);
        renderPreviousButton(html, getPreviusUrl(), renderGlyphiconPrevius(), null, styleButtonDefault, null);
        renderScrollField(html);
        renderNextButton(html, getNextUrl(), renderGlyphiconNext(), null, styleButtonDefault, null);
        renderCloseDIV(html);
        html.append("</div>");
        renderScrollOtherField(html);
        renderCloseDIV(html);
        renderOpenDIV(html,"clearfix",null);
        renderCloseDIV(html);
    }
    ...
    protected void renderInputText(StringBuffer html, String name, int value, String size, String maxLength) {
        ...
        html.append(" type=\"text\" class=\""+ StyleClassFunctions.getFormInputClasses()+"\"");
    ...
    protected void renderMonthSelect(StringBuffer html, int selected) {
        ...
        html.append(" name=\"").append(name).append("\"").append(" id=\"").append(name).append("\"").append(" class=\""+StyleClassFunctions.getFormSelectClasses()+"\" >\n");
