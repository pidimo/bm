...
// Creates a new plugin class and a custom listbox menu
tinymce.create('tinymce.plugins.ElwisMenu', {
    ...
    init : function(ed, url) {
        //here define the button text
        ed.onInit.add(function(ed) {
            var DOMElementNodeType = 1;
            var textAreaElement = ed.getElement();
            for (var i in oElwisConfig.buttonList) {
                var btn = oElwisConfig.buttonList[i];
                if (btn) {
                    var elBtn = null;
                    //TinyMCE define button element id as {containerId}_{buttonControlId}
                    if (textAreaElement.id) {
                        elBtn = tinymce.DOM.get(textAreaElement.id + "_" + btn.id);
                    } else if (textAreaElement.name) {
                        elBtn = tinymce.DOM.get(textAreaElement.name + "_" + btn.id);
                    }
                    if (elBtn) {
                        //remove some class to clearly view
                        tinymce.DOM.removeClass(elBtn, "mceButton");

                        while (elBtn.nodeType == DOMElementNodeType && elBtn.firstChild) {
                            elBtn = elBtn.firstChild;
                            tinymce.DOM.removeClass(elBtn, "mceIcon");
                        }
                        //define text for button
                        tinymce.DOM.setHTML(elBtn, btn.resource);
                    }
                }
            }
        });

        // Add a button like date button or showHTML button
        for (var i in oElwisConfig.buttonList) {
            var btn = oElwisConfig.buttonList[i];
            if (btn) {
                if (btn && btn.type == ElwisConfig.buttonType) {
                    var variable = oElwisConfig.buildVariable(btn.varId, btn.varResource);
                    ed.addButton(btn.id, {
                        title : btn.tooltip,
                        'class': 'elwisButton',
                        onclick : function() {
                ...
                } else if (btn.type == "showHtml") {
                    ed.addButton(btn.id, {
                        title : btn.tooltip,
                        'class': 'elwisButton',
                        onclick : function() {
                            //alert(tinyMCE.activeEditor.getBody().innerHTML);
                            alert(ed.getBody().innerHTML);
                        }
                    });
                }
            }
        ...
        for (var i in oElwisConfig.buttonList) {
            ...
            if (btn && btn.id == n) {
                if (btn.type == ElwisConfig.menuType) {
                    ...
                    var menuBtnControl = cm.createMenuButton(btn.id, {
                        title : btn.tooltip,
                        icons : false,
                        'class': 'elwisButton'
                    });

                    //alert(menuBtnControl.renderHTML());
                    menuBtnControl.onRenderMenu.add(function(menuBtnControl, menu) {
                        var sub;

                        if (oElwisConfig.itemList[btn.id]) {
                            var menuList = oElwisConfig.itemList[btn.id];
                            for (var j in menuList) {
                                var item = menuList[j];

                                if (item.isSubMenu) {
                                    sub = menu.addMenu({title : item.resource});
                                    buildSubMenu(item.id, sub);

                                } else {
                                    menu.add({title : item.resource, idVar: item.varId, resourceVar : item.varResource, onclick : function(e) {
                                        var variable = oElwisConfig.buildVariable(this.idVar, this.resourceVar);
                                        tinyMCE.activeEditor.execCommand('mceInsertContent', false, variable);
                                        tinyMCE.activeEditor.focus();
                                    }});
                                }
...
tinymce.PluginManager.add('elwisplugin', tinymce.plugins.ElwisMenu);
/***************************************************************/
/***************************************************************/
//external functions
...
/**
 * build sub menu listbox
 */
function buildSubMenu(itemId, sub) {

    if (oElwisConfig.itemList[itemId]) {
        var menuList = oElwisConfig.itemList[itemId];
        var subMenuChild
        for (var j in menuList) {
            var item = menuList[j];

            if (item.isSubMenu) {
                subMenuChild = sub.addMenu({title : item.resource});

                buildSubMenu(item.id, subMenuChild);
            } else {
                sub.add({title : item.resource, idVar: item.varId, resourceVar : item.varResource, onclick : function(e) {
                    var variable = oElwisConfig.buildVariable(this.idVar, this.resourceVar);
                    tinyMCE.activeEditor.execCommand('mceInsertContent', false, variable);
                    tinyMCE.activeEditor.focus();
                }});
