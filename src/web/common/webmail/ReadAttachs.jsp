...
<c:choose>
    <c:when test="${param.iss}">
        <c:set var="attachs_" value="${caseActivityForm.dtoMap.attachs}"/>
    </c:when>
    <c:when test="${null != formName}">
        <c:set var="attachs_" value="${mainCommunicationForm.dtoMap.attachs}"/>
    </c:when>
    <c:otherwise>
        <c:set var="attachs_" value="${dto.attachs}"/>
    </c:otherwise>
</c:choose>
                    ...
                    <c:if test="${attachs.attachVisibility!=true}">
                        ...
                        <tr>
                            <td width="5%">
                                ...
                                <app:link action="/webmail/Mail/Download.do?dto(attachId)=${attachs.attachId}" contextRelative="true">
