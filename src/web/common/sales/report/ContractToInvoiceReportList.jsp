...
<tr>
    ...
    <td class="label" width="12%"><fmt:message key="Contract.payMethod"/></td>
    <td class="contain" width="38%">
        <html:select property="parameter(payMethod)" styleClass="middleSelect" styleId="payMethodId"
                     tabindex="7">
            <html:option value=""/>
...
<tr>
    ...
    <td class="label"><fmt:message key="ContractToInvoice.report.calculateTotalAs"/></td>
    <td class="contain">
        <c:set var="contractIncomeList" value="${app2:getContractIncomeTypes(pageContext.request)}"/>
        <html:select property="parameter(contractIncome)" styleClass="middleSelect" tabindex="9">
...
<tr>
    ...
    <td class="label"><fmt:message key="ContractToInvoice.report.withoutAlreadyInvoiced"/></td>
    <td class="contain">
        <html:checkbox property="parameter(withoutInvoiced)" styleId="allContacts" styleClass="radio" tabindex="10">
        </html:checkbox>
    </td>
</tr>
<tr>
    <td class="label"><fmt:message key="ProductContract.seller"/></td>
    <td class="contain">
    ...
    <td class="contain">
        ...
        &nbsp;
        <app:dateText property="parameter(fromDate)" maxlength="10" tabindex="11" styleId="fromDate" 
                      calendarPicker="true" datePatternKey="${datePattern}" styleClass="dateText" convert="true"/>
        &nbsp;
        <fmt:message key="Common.to"/>
        &nbsp;
        <app:dateText property="parameter(toDate)" maxlength="10" tabindex="12" styleId="toDate"
                      calendarPicker="true" datePatternKey="${datePattern}" styleClass="dateText" convert="true"/>
    </td>
</tr>
...
<titus:reportTag id="contractInvoiceReportList" title="SalesProcess.Report.ContractToInvoiceList"
    ...
    <titus:reportFieldTag name="contractNumber" resourceKey="ProductContract.contractNumber" type="${FIELD_TYPE_STRING}" width="10" fieldPosition="1"/>
    <titus:reportFieldTag name="addressName" resourceKey="Contract.contact" type="${FIELD_TYPE_STRING}" width="15" fieldPosition="2"/>
    <titus:reportFieldTag name="customerName" resourceKey="Sale.customer" type="${FIELD_TYPE_STRING}" width="15" fieldPosition="3"/>
    <titus:reportFieldTag name="productName" resourceKey="Contract.product" type="${FIELD_TYPE_STRING}" width="15" fieldPosition="4"/>
    <titus:reportFieldTag name="payStartDate" resourceKey="ProductContract.payStartDate" type="${FIELD_TYPE_DATEINT}" width="10" fieldPosition="5" patternKey="datePattern"/>
    <titus:reportFieldTag name="contractEndDate" resourceKey="ContractToInvoice.report.contractEndDate" type="${FIELD_TYPE_DATEINT}" width="10" fieldPosition="6" patternKey="datePattern"/>
    <titus:reportFieldTag name="contractIncomeVar" resourceKey="ContractToInvoice.report.incomeCost" type="${FIELD_TYPE_CALCULATEDDECIMALNUMBER}" width="15" fieldPosition="7"
                          conditionMethod="com.piramide.elwis.utils.ReportHelper.getAsDouble (contractIncomeVar)" align="${FIELD_ALIGN_RIGHT}" patternKey="numberFormat.2DecimalPlaces"/>
    <titus:reportFieldTag name="currencyLabel" resourceKey="ProductContract.currency" type="${FIELD_TYPE_STRING}" width="10" fieldPosition="8"/>
</html:form>
</table>
