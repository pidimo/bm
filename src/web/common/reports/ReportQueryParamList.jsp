...
<table id="ReportFilterList.jsp" border="0" cellpadding="0" cellspacing="0" width="80%" class="container" align="center">
    <tr>
        <td>
            <fanta:table list="reportQueryParamList" width="100%" id="listF" action="Report/QueryParam/List.do" imgPath="${baselayout}"
                         align="center">

                <app:url var="urlEdit"
                         value="/Report/QueryParam/Forward/Update.do?dto(reportQueryParamId)=${listF.reportQueryParamId}&dto(parameterName)=${app2:encode(listF.parameterName)}&dto(op)=readAll" enableEncodeURL="false"/>
                ...
                <fanta:dataColumn name="parameterName" title="Report.queryParam.parameterName" action="${urlEdit}" styleClass="listItem"
                ...
                <fanta:dataColumn name="" title="Report.filter.value" styleClass="listItem2" renderData="false"
                    ...
                    <c:if test="${not empty listF.filterId}">
                        <c:set var="values" value="${app2:getValuesOfFilter(listF.filterId,listF.filterType,pageContext.request)}"/>
