...
<textarea id="mt" rows="10" cols="80" style="display:none"></textarea>
...
<app2:jScriptUrl url="/titus.html?locale=${sessionScope.user.valueMap['locale']}&op=reload&name=${param.reportId}&dynamicParam=${paramDynamicColumn}" var="jsReloadTreeUrl" addModuleParams="false">
...
<app2:jScriptUrl url="${ajaxUrlExecute}?reportId=${param.reportId}&command=filter" var="jsAjaxUrl" addModuleParams="false">
...
<table>
    <tr><td width="500">
        ...
        <div class="dtree" id="dtree" style="overflow:scroll; width:400; height:250;">
            ...
            <script type="text/javascript">
                ...
                function reload(path, id, paths, topOpenPaths) {
                    <%--var url = 'titus.html?locale=<%=Config.get(request.getSession(), Config.FMT_LOCALE).toString()%>'+
                    '&op=reload&jsPath=' + path +"&paths="+ paths + "&topOpenPaths=" + topOpenPaths+"&name=${param.reportId}";--%>
                ...
                function doit(xmlTree) {
                    ...
                    for (j = 0; j < nodes.length; j++) {
                        ...
                        //logDiv.innerHTML = logDiv.innerHTML + "id:" + id + " - parentId:" + parentId + " - label:" + label + " - method:" + method + "<br>";
                ...
                function doSomething(nodeId) {
                    ...
                    <%--var url = '${ajaxUrlExecute}' + '?command=filter&jsPath=' + escape(d.getPath(nodeId)) + "&reportId=${param.reportId}&date="+new Date().getTime();--%>
                    var url = ${jsAjaxUrl};
                    requestCount++;
        ...
        <td width="300"><div id="divView"></div></td>
    </tr>
</table>
