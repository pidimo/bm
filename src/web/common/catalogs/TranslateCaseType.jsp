...
<tr>
    <td>
    <html:form action="/Support/CaseType/Translate/Save.do">

        <html:hidden property="dto(op)" value="update"/>
        <html:hidden property="dto(companyId)" value="${sessionScope.user.valueMap['companyId']}"/>
        <html:hidden property="dto(caseTypeId)" value="${caseTypeTranslateForm.dtoMap['caseTypeId']}"/>
        <html:hidden property="dto(langTextId)" value="${caseTypeTranslateForm.dtoMap['langTextId']}"/>
        <html:hidden property="dto(caseTypeName)" value="${caseTypeTranslateForm.dtoMap['caseTypeName']}"/>

        <c:set var="disableChecks" value=""/>
        <c:if test="${caseTypeTranslateForm.dtoMap['haveDefaultTranslation'] == true}">
        ...
        <tr>
            <td colspan="3" class="title"><fmt:message  key="CaseType.Title.translate"/></td>
        </tr>
        <tr>
        ...
        <c:forEach var="langText" items="${caseTypeTranslateForm.dtoMap['translatedSystemLanguages']}" varStatus="counter" >
        <tr>
