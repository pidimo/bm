...
<tr>
    <td>
        <html:form action="${action}" focus="dto(typeName)" >

        <table id="ProductType.jsp" border="0" cellpadding="0" cellspacing="0" width="60%" align="center" class="container">
    ...
    <c:if test="${'delete' == op}">
        ...
        <TR>
        <TD class="label" width="25%" nowrap><fmt:message  key="ProductType.typeName"/></TD>
        <TD class="contain" width="75%">
        <app:text styleClass="largetext" property="dto(typeName)" maxlength="80" view="${'delete' == op}"/>
        </TD>
        </TR>
        <tr>
...
<%--CREATE, CANCEL, SAVE AND NEW buttons--%>
    ...
    <TR>
        <TD class="button">
        <app2:securitySubmit operation="${op}" functionality="PRODUCTTYPE" styleClass="button">${button}</app2:securitySubmit>
        <c:if test="${op == 'create'}" >
            <app2:securitySubmit operation="${op}" functionality="PRODUCTTYPE" styleClass="button" property="SaveAndNew"><fmt:message   key="Common.saveAndNew"/></app2:securitySubmit>
        </c:if>
