...
<tr>
    ...
    <TABLE id="VatList.jsp" border="0" cellpadding="0" cellspacing="0" width="60%" class="container" align="center">
    <TR align="center">
    <td>
        <fanta:table list="vatList" width="100%" id="vat" action="Vat/List.do" imgPath="${baselayout}"  >
            <c:set var="editAction" value="Vat/Forward/Update.do?dto(vatId)=${vat.id}&dto(vatLabel)=${app2:encode(vat.name)}"/>
            <c:set var="deleteAction" value="Vat/Forward/Delete.do?dto(withReferences)=true&dto(vatId)=${vat.id}&dto(vatLabel)=${app2:encode(vat.name)}"/>
            ...
            <fanta:dataColumn name="name" action="${editAction}" styleClass="listItem" title="Vat.name"  headerStyle="listHeader" width="30%" orderable="true" maxLength="25" />
            <fanta:dataColumn name="description" styleClass="listItem2" title="Vat.description"  headerStyle="listHeader" width="65%" orderable="true" maxLength="40"/>
        </fanta:table>
    </td>
    </tr>
    <tr>
