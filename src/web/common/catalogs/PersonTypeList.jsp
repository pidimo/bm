...
<app2:checkAccessRight functionality="PERSONTYPE" permission="CREATE">
    ...
    <TABLE id="PersonTypeList.jsp" border="0" cellpadding="0" cellspacing="0" width="60%" class="container" align="center">
    <TR align="center" height="20">
        <TD>
            <fanta:table list="personTypeList" width="100%" id="personType" action="PersonType/List.do" imgPath="${baselayout}"  >
                <c:set var="editAction" value="PersonType/Forward/Update.do?dto(personTypeId)=${personType.id}&dto(personTypeName)=${app2:encode(personType.name)}"/>
                <c:set var="deleteAction" value="PersonType/Forward/Delete.do?dto(withReferences)=true&dto(personTypeId)=${personType.id}&dto(personTypeName)=${app2:encode(personType.name)}"/>
                ...
                <fanta:dataColumn name="name" action="${editAction}" styleClass="listItem2" title="PersonType.name"  headerStyle="listHeader" width="95%" orderable="true" maxLength="25" >
                </fanta:dataColumn>
            </fanta:table>
        </TD>
    </tr>
