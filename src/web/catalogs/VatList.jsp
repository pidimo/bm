...
<c:set var="helpResourceKey" value="help.config.finances.vat.list" scope="request"/>

<fmt:setBundle basename="com.piramide.elwis.web.resources.ApplicationResources"/>
<c:set var="windowTitle" value="Vat.plural" scope="request"/>
<c:set var="pagetitle" value="Common.configuration" scope="request"/>

<c:choose>
    <c:when test="${sessionScope.isBootstrapUI}">
        <c:set var="body" value="/WEB-INF/jsp/catalogs/VatList.jsp" scope="request"/>
        <c:set var="tabs" value="/WEB-INF/jsp/layout/tiles/CatalogTabs.jsp" scope="request"/>
        <c:import url="/WEB-INF/jsp/layout/ui/main.jsp"/>
    </c:when>
    <c:otherwise>
        <c:set var="body" value="/common/catalogs/VatList.jsp" scope="request"/>
