...
<c:choose>
    <c:when test="${category == '1' || param.category == null}">
                <c:set target="${tabItems}" property="User.Preferences"  value="/UserPreferences/User.do?dto(userId)=${user.valueMap['userId']}"/>
                <c:set target="${tabItems}" property="User.password"  value="/UserPreferences/UserPassword.do?dto(userId)=${user.valueMap['userId']}"/>
...
<app2:checkAccessRight functionality="USERINTERFACE" permission="VIEW">
        ...
        <c:set target="${tabItems}" property="UIManager.Styles"  value="/UIManager/Forward/UserStyleSheet.do?paramSection=${app2:encode(firstSection)}"/>
</app2:checkAccessRight>
