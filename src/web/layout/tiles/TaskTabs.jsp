...
<app2:checkAccessRight functionality="TASK" permission="VIEW">
    <c:set target="${tabItems}" property="Appointment.Tab.detail"  value="/Task/Forward/Update.do?taskId=${taskId}&dto(title)=${app2:encode(title1)}&update=true&dto(recordUserId)=${sessionScope.user.valueMap['userId']}"/>
</app2:checkAccessRight>
<app2:checkAccessRight functionality="TASKUSER" permission="VIEW">
    <c:if test="${sessionScope.user.valueMap['userId'] == userId}">
        <c:set target="${tabItems}" property="Scheduler.Task.users"  value="/Task/ParticipantTaskList.do?taskId=${taskId}&dto(title)=${app2:encode(title1)}"/>
    </c:if>
</app2:checkAccessRight>
