...
<html:form action="/Support/CaseSeverity/Translate/Save.do" styleClass="form-horizontal translate">
    <div class="${app2:getFormClasses()}">
        <html:hidden property="dto(op)" value="update"/>
        <html:hidden property="dto(companyId)" value="${sessionScope.user.valueMap['companyId']}"/>
        <html:hidden property="dto(severityId)" value="${caseSeverityTranslateForm.dtoMap['severityId']}"/>
        <html:hidden property="dto(langTextId)" value="${caseSeverityTranslateForm.dtoMap['langTextId']}"/>
        <html:hidden property="dto(severityName)" value="${caseSeverityTranslateForm.dtoMap['severityName']}"/>
        <c:set var="disableChecks" value=""/>
        <c:if test="${caseSeverityTranslateForm.dtoMap['haveDefaultTranslation'] == true}">
            <c:set var="disableChecks" value="disabled"/>
        </c:if>
        <div class="${app2:getFormPanelClasses()}">
            <fieldset>
                <legend class="title">
                    <fmt:message key="CaseSeverity.Title.translate"/>
                </legend>
            </fieldset>
            <div class="table-responsive">
                <table class="${app2:getTableClasesIntoForm()}">
                    ...
                    <tbody>
                    <c:forEach var="langText" items="${caseSeverityTranslateForm.dtoMap['translatedSystemLanguages']}"
                               varStatus="counter">
                        <tr>
                            <td>
...
<tags:jQueryValidation formName="caseSeverityTranslateForm"/>