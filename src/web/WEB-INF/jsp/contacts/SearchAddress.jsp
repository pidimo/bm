...
<%--compose search url paramas--%>
<c:set var="searchUrl" value="SearchAddress.do" scope="page"/>
<c:if test="${not empty param.action}">
    <c:set var="searchUrl" value="${param.action}" scope="page"/>
</c:if>

<%--add allow creation param--%>
<c:set var="searchUrl" value="${searchUrl}?allowCreation=${param.allowCreation}" scope="page"/>

<%--add select field params--%>
<c:set var="parentAddressIdStyleId" value="fieldAddressId_id"/>
<c:if test="${not empty param.addressIdStyleId}">
    <c:set var="parentAddressIdStyleId" value="${param.addressIdStyleId}"/>
    <c:set var="searchUrl" value="${searchUrl}&addressIdStyleId=${param.addressIdStyleId}" scope="page"/>
</c:if>

<c:set var="parentNameStyleId" value="fieldAddressName_id"/>
<c:if test="${not empty param.addressNameStyleId}">
    <c:set var="parentNameStyleId" value="${param.addressNameStyleId}"/>
    <c:set var="searchUrl" value="${searchUrl}&addressNameStyleId=${param.addressNameStyleId}" scope="page"/>
</c:if>


<script>
    function select(id, name) {
        parent.selectField('${parentAddressIdStyleId}', id, '${parentNameStyleId}', name);
    }
...
<div class="${app2:getListWrapperClasses()}">
    <html:form action="${searchUrl}" focus="parameter(contactSearchName)"
    ...
    <div class="${app2:getAlphabetWrapperClasses()}">
        <fanta:alphabet action="${searchUrl}" parameterName="name1"
    ...
    <div>
        ...
        <div class="table-responsive">
            <fanta:table mode="bootstrap" width="100%" id="contact" styleClass="${app2:getFantabulousTableClases()}" action="${searchUrl}"
                         imgPath="${baselayout}">
                ...
                <fanta:dataColumn name="${columnName}" styleClass="listItem" title="Contact.name"
                                  headerStyle="listHeader" width="41%" orderable="true" maxLength="40"/>
