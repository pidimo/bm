...
<div class="${app2:getListWrapperClasses()}">
    ...
    <div class="table-responsive" id="tableId">
        <fanta:table mode="bootstrap" list="importRecordContactPersonDupliList"
                     width="100%"
                     id="importRecord"
            ...
            <c:choose>
                <c:when test="${isContactPersonDedupli}">
                    ...
                    <app:url var="editLink"
                             value="ImportRecord/ContactPerson/Forward/Deduplication.do?importRecordId=${importRecord.importRecordId}&dto(importRecordId)=${importRecord.importRecordId}&dto(profileId)=${param.profileId}&dto(organizationId)=${importRecord.organizationId}&dto(parentRecordId)=${importRecord.parentRecordId}"/>
                    ...
                    <fanta:dataColumn useJScript="true" action="javascript:goParentURL('${editLink}')"
                                      name="addressName" styleClass="listItem"
                                      title="ImportRecord.dupli.contactPerson.name"
                                      headerStyle="listHeader" orderable="true" width="65%"/>
                </c:when>
                <c:otherwise>
                    <fanta:dataColumn name="addressName" styleClass="listItem"
                                      title="ImportRecord.dupli.contactPerson.name"
