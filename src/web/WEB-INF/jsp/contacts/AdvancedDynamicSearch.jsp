...
<c:choose>
    <c:when test="${'contactSearchName' eq fieldAlias}">
        <html:text property="parameter(contactSearchName)" styleClass="largeText ${app2:getFormInputClasses()}"/>
    </c:when>
    ...
    <c:when test="${'addressTypeA1' eq fieldAlias}">
        <html:select property="parameter(addressTypeA1)"
                     styleClass="mediumSelect ${app2:getFormSelectClasses()}">
            <html:option value=""/>
    ...
    <c:when test="${'customerTypeId' eq fieldAlias}">
        <app:catalogSelect property="parameter(customerTypeId)"
    ...
    <c:when test="${'sourceId' eq fieldAlias}">
        <app:catalogSelect property="parameter(sourceId)"
                           catalogTable="addresssource" idColumn="sourceid"
                           labelColumn="sourcename"
                           styleClass="mediumSelect ${app2:getFormSelectClasses()}"/>
    </c:when>
    <c:when test="${'payConditionId' eq fieldAlias}">
        <app:catalogSelect property="parameter(payConditionId)"
    ...
    <c:when test="${'payMoralityId' eq fieldAlias}">
        <app:catalogSelect property="parameter(payMoralityId)"
    ...
    <c:when test="${'partnerId' eq fieldAlias}">
        ...
        <div class="input-group">
            <app:text property="parameter(partnerName)"
            ...
            <span class="input-group-btn">
                <tags:bootstrapSelectPopup styleId="searchPartner_id"
                                           ...
                                           name="searchPartner" titleKey="Common.search"/>
    ...
    <c:when test="${'departmentId' eq fieldAlias}">
        <fanta:select property="parameter(departmentId)"
                      ...
                      firstEmpty="true" labelProperty="name"
                      valueProperty="departmentId" styleClass="mediumSelect ${app2:getFormSelectClasses()}">
            <fanta:parameter field="companyId" value="${sessionScope.user.valueMap['companyId']}"/>
        </fanta:select>
    </c:when>
    <c:when test="${'personTypeId' eq fieldAlias}">
        <fanta:select property="parameter(personTypeId)"
                      listName="personTypeList" firstEmpty="true"
                      labelProperty="name"
