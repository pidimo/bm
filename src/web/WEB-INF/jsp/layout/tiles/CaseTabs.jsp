...
<c:set var="caseId"><%=request.getParameter("caseId")%>
</c:set>
...
<app2:checkAccessRight functionality="CASE" permission="VIEW">
    <c:set target="${tabItems}" property="Common.detail"
           value="/Case/Forward/Update.do?dto(caseTitle)=${app2:encode(tabHeaderValue)}"/>
</app2:checkAccessRight>

<app2:checkAccessRight functionality="CASEACTIVITY" permission="VIEW">
    <c:set target="${tabItems}" property="SupportCaseActivity.myTitle.plural"
           value="/CaseActivity/List.do?n=1&dto(caseTitle)=${app2:encode(tabHeaderValue)}"/>
</app2:checkAccessRight>
...
<c:if test="${sessionScope.user.valueMap.userType != 0}">
    <app2:checkAccessRight functionality="COMMUNICATION" permission="VIEW">
        <c:set target="${tabItems}" property="Contacts.Tab.communications"
               value="/Communication/List.do?dto(caseTitle)=${app2:encode(tabHeaderValue)}"/>
    </app2:checkAccessRight>
</c:if>

<app2:checkAccessRight functionality="SUPPORTATTACH" permission="VIEW">
    <c:set target="${tabItems}" property="Article.Tab.Attachments"
           value="/CaseAttach/List.do?dto(caseTitle)=${app2:encode(tabHeaderValue)}"/>
</app2:checkAccessRight>
